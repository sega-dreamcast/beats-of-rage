...
//printf("opengif(filename=\"%s\", packfilename=\"%s\")\n", filename, packfilename);

	if((handle=openpackfile(filename,packfilename))==-1) {
//printf("openpackfile failed\n");
          return 0;
        }

//printf("ok 1\n");
//printf("incidentally, sizeof(gifheaderstruct) = %d and sizeof_gifheaderstruct = %d\n", sizeof(gifheaderstruct), sizeof_gifheaderstruct);
...
//printf("openimage(filename=\"%s\", packfile=\"%s\"\n", filename, packfile);
...
//printf("loadscreen(filename=\"%s\", packfile=\"%s\", pal)\n", filename, packfile);

	if(!openimage(filename, packfile)) {
//          printf("loadscreen: openimage failed\n");
          return NULL;
        }
